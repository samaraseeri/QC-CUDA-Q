...
try:
    ...
    # If CUDA-Q is available, define the real kernel with CORRECTED syntax
    @cudaq.kernel
    def _real_cudaq_vqc_kernel(num_qubits: int, depth: int, theta: list[float]):
        q = cudaq.qvector(num_qubits)  # FIXED: Use qvector instead of qalloc
        param_idx = 0
        for _ in range(depth):
            for i in range(num_qubits):
                ry(theta[param_idx], q[i])  # FIXED: Use ry directly, not cudaq.ry
                param_idx += 1
            for i in range(num_qubits - 1):
                cx(q[i], q[i + 1])  # FIXED: Use cx directly, not cudaq.cx
...
if __name__ == '__main__':
    ...
    print("\nPlot saved as 'ode_solutions.png'")
    
    print(f"\n[CUDA-Q Status] CUDA-Q Available: {_CUDAQ_AVAILABLE}")
    if _CUDAQ_AVAILABLE:
        print("[CUDA-Q Status] Using real CUDA-Q quantum circuits")
    else:
        print("[CUDA-Q Status] Using fallback simulation (random amplitudes)")
